...
(async () => {
  const list = await (await fetch(`http://127.0.0.1:${process.env.PANO_PLAYER_CDP_PORT || 19338}/json/list`)).json();
  const target = list.find(x => x.type === 'iframe' && x.url.includes('shijunjie.pano-player'));
  if (!target) throw new Error('PanoPlayer Webview not found');
  ...
  if (process.env.PANO_PLAYER_NETWORK) await send('Network.enable', {});
  const result = await send('Runtime.evaluate', { expression, returnByValue: true, awaitPromise: true, userGesture: true });
  const value = result.result?.result?.value;
  if (process.env.PANO_PLAYER_REPORT && value) await fs.writeFile(process.env.PANO_PLAYER_REPORT, JSON.stringify(value, null, 2));
  if (value?.samples) {
  ...
  if (process.env.PANO_PLAYER_NETWORK) {
