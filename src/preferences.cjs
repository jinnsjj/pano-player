...
const defaults = Object.freeze({
  ...
  volume: 1, muted: false, grid: false, gridOpacity: 1, yaw: 0, pitch: 0, fov: 72,
});
const choices = {
...
function normalize(value) {
  ...
  for (const key of ['enabled', 'muted', 'grid']) if (typeof value[key] === 'boolean') result[key] = value[key];
  return result;
}
